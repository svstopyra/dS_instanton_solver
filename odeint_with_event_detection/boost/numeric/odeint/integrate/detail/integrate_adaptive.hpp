/*
 ...
 [begin_description]
 Default Integrate adaptive implementation, modified to include early termination with an event detecting observer.
 [end_description]
...
)
{
    ...
    while( less_with_sign( start_time , end_time , dt ) && !terminate_early)
    {
		//Call the observer, returning true if the event detection determines we ought  to terminate early:
        terminate_early = obs( start_state , start_time );
		if(terminate_early)
		{
			break;			
		}
		
		//Proceed with the rest of the odeint code:
	...
	if(!terminate_early)
	{
		//Call the observer for the final step, unless we decided to exit early.
        ...
        Observer observer , dense_output_stepper_tag )
{
	...
	bool terminate_early = false;
    while( less_with_sign( st.current_time() , end_time , st.current_time_step() ) && !terminate_early)
    {
        while( less_eq_with_sign( static_cast<Time>(st.current_time() + st.current_time_step()) ,
               end_time ,
               st.current_time_step() ) && !terminate_early)
        {   //make sure we don't go beyond the end_time
            terminate_early = obs( st.current_state() , st.current_time() );
            st.do_step( system );
            ++count;
        }
